				...
				<div class="table-responsive">
					<table class="table table-striped table-bordered table-hover">
						...
						<?php
							// Assign a query
							if($status=="petugas"){
								$query = "SELECT * FROM pkt INNER JOIN mahasiswa ON pkt.nim=mahasiswa.nim ORDER BY pkt.flag_lab LIMIT 10";
							}elseif ($status=="dosen") {
								$query = "SELECT * FROM pkt INNER JOIN mahasiswa ON pkt.nim=mahasiswa.nim WHERE pkt.nip='".$dosen->nip."'";
							}elseif ($status=="lab"){
								$query = "SELECT * FROM pkt INNER JOIN mahasiswa ON pkt.nim=mahasiswa.nim INNER JOIN lab on pkt.flag_lab=lab.nama_lab WHERE pkt.flag_lab='".$lab->nama_lab."'";
							}elseif ($status=="anggota"){
								$query = "SELECT * FROM pkt INNER JOIN mahasiswa ON pkt.nim=mahasiswa.nim WHERE mahasiswa.nim='".$anggota->nim."'";
							}
