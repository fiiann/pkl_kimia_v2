				...
				<div class="table-responsive">
					<table class="table table-striped table-bordered table-hover">
						...
						<?php
							// Assign a query
							if ($status=="petugas") {
								$query = "SELECT * FROM daftar_tr1 INNER JOIN anggota ON daftar_tr1.nim=anggota.nim INNER JOIN lab ON anggota.idlab=lab.idlab ORDER BY lab.id_lab LIMIT 10";
							}elseif ($status=="dosen") {
								$query = "SELECT * FROM daftar_tr1 INNER JOIN anggota ON daftar_tr1.nim=anggota.nim INNER JOIN lab ON anggota.idlab=lab.idlab WHERE anggota.id_wali=dosen.id_wali";
							}
							...
							while($row = $result->fetch_object()){
								...
								echo "<td>
										<a href='input_judul_pkt.php?nim=".$row->nim."'><button class='btn btn-info'>Input Judul Outline</button></a>
									 </td>";
								if ($status=="petugas") {
