<?php
	...
	if (!isset($_POST['daftar'])) {
		...
		$outline=test_input($_POST['outline']);
		if ($outline=='') {
			$errorOutline='wajib diisi';
			$validOutline=FALSE;
		}else{
			$validOutline=TRUE;
		}
		...
		if ($validNim && $validOutline && $validProgress && $validLaporan && $validKinerja) {
			$nim=$con->real_escape_string($nim);
			$outline=$con->real_escape_string($outline);
			$progress=$con->real_escape_string($progress);	
			...
			$query = "UPDATE nilai_tr1 SET  outline=$outline, progress=$progress, laporan=$laporan, kinerja=$kinerja, huruf='".$huruf."',nilai_akhir=$nilai_akhir WHERE nim='".$nim."'";
					...
					<div class="col-md-12">
						<form method="POST" role="form" autocomplete="on" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
							...
							<div class="form-group">
								...
								<input class="form-control" type="text" name="outline" maxlength="50" size="30" placeholder="nilai outline" required value="<?php if(isset($nilai_total)){echo $nilai_total;} ?>">
							</div>

							<div class="form-group">
								<label>Nilai Progress</label>&nbsp;<span class="label label-warning">* <?php if(isset($errorProgress)) echo $errorProgress;?></span>
								<input class="form-control" type="text" name="progress" maxlength="50" size="30" placeholder="nilai progress" required value="<?php if(isset($jumlah_total)){echo $jumlah_total;} ?>">
							</div>
