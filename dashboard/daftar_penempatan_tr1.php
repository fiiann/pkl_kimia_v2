				...
				<div class="table-responsive">
					<table class="table table-striped table-bordered table-hover">
						...
						<?php
							// Assign a query
							if ($status=="petugas") {
								$query = "SELECT * FROM tr1 t INNER JOIN mahasiswa m ON t.nim=m.nim  ORDER BY m.nama LIMIT 10";
							}elseif ($status=="dosen"){
								$query = "SELECT * FROM tr1 t INNER JOIN mahasiswa m ON t.nim=m.nim  INNER JOIN lab l ON t.flag_lab=l.nama_lab WHERE p.flag_lab='".$dosen->idlab."'";
							}elseif ($status=="lab"){
								$query = "SELECT * FROM tr1 t INNER JOIN mahasiswa m ON t.nim=m.nim  WHERE t.flag_lab='".$lab->nama_lab."'";
							}else {
