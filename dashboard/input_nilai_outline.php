<?php
	...
	if (!isset($_POST['daftar'])) {
		...
		$query = " SELECT * FROM mahasiswa m WHERE m.nim='".$nim."'";
		// Execute the query
		...
		if ($nim=='') {
			...
			$query = " SELECT * FROM outline WHERE nim='".$nim."'";
			$result = $con->query( $query );
		...
		if ($validNim && $validnilai_identifikasi && $valid_rumusan && $valid_tujuan && $valid_metodologi && $valid_hipotesis && $valid_analisis && $valid_kontrak && $valid_tanggal) {
			...
			$query = "INSERT INTO outline (nim, identifikasi, rumusan, tujuan, metodologi, hipotesis, analisis, kontrak,nilai_total,jadwal) VALUES ('".$nim."',$nilai_identifikasi,$rumusan,$tujuan,$metodologi, $hipotesis, $analisis, $kontrak, $akhir, '".$tanggal."')";
					...
					<div class="col-md-12">
						<form method="POST" role="form" autocomplete="on" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
							...
							<div class="form-group">
								...
								<input class="form-control" readonly type="text" name="nim" maxlength="14" size="30" placeholder="nim 14 digit angka" required autofocus value="<?php if(isset($nim)){echo $nim;} ?>">
							</div>
							<div class="form-group">
